...
const AssignAsset = () => {
  ...
  const handleAssign = async (e) => {
    ...
    const employee = employees.find(
      (emp) => emp.employeeEmail === selectedEmployee
    );
    ...
    try {
      ...
      toast.success(
        `Assigned ${asset.productName} to ${employee.employeeName}`
      );
      navigate("/dashboard/asset-lists");
    } catch (error) {
      console.error(error);
        ...
        <div className="space-y-4">
          ...
          <div className="card bg-base-200 p-4">
            ...
            <p className="text-sm font-semibold">
              Available Quantity:{" "}
              <span className="text-primary">{asset?.availableQuantity}</span>
            </p>
          </div>
        </div>
