...
import { FaSearch, FaFilePdf, FaUndoAlt, FaClock, FaCheckCircle } from "react-icons/fa";
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Track and manage your requested company property.</p>
        </div>
          ...
          <table className="table table-zebra w-full">
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan="6" className="text-center py-10 text-gray-400">No assets found matching your criteria.</td></tr>
              ) : (
                assets.map((asset) => (
                  <tr key={asset._id} className="hover:bg-base-200/50 transition-colors">
                    <td className="font-bold text-neutral">{asset.assetName}</td>
                    <td>
                      <span className={`badge badge-sm font-semibold ${asset.assetType === 'Returnable' ? 'badge-ghost' : 'badge-outline'}`}>
                        {asset.assetType}
                      </span>
                    </td>
